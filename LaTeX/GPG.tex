	...
	\newpage
		...
		\section{GPG}\label{GPG}
			...
			\subsection{Wat is GPG?}
				...
				GnuPG is een commando voor de terminal, Te zien in Subsectie~\ref{CLI}, maar er zijn 					dergelijke frontend\index{Frontend} programma's om deze in een gui te kunnen gebruiken. 				Te zien in Subsectie~\ref{GUI}

			\subsection{GPG in command-line interface (CLI)}\label{CLI}
				Zoals ieder ander command, heeft GPG ook zijn nodige syntax\index{Syntax}.\\
				\begin{center}
			...
			\subsection{Frontend GPG programma's}\label{GUI} \cite{Frontend} 
				...
				\begin{figure}[!ht]				
					...
					\paragraph{Seahorse} \cite{Seahorse}
						Seahorse is een GUI voor GNOME. Het is ook geintegreerd in Nautilus\index{Nautilus}, gedit 								en andere applicaties voor encryptie uit te voeren.
						Je kan met Seahorse; PGP en SSH\index{SSH} sleutels maken en beheren, publiceren en 								terughalen van sleutels op de servers, een passphrase cachen, 											sleutelhanger backuppen, enz...
					\begin{center}
